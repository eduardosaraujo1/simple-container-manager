...
**Descrição:**Por se tratar de uma interface de fácil uso, o sistema só deve listar alguns Docker Containers, mas não todos. Por isso, através do arquivo de configuração, o administrador poderá especificar um ícone e uma label (etiqueta) a ser exibida como nome da aplicação. Essa configuração DEVE persistir entre inicializações do aplicativo.
...
- [x] Definir escopo
- [ ] Estruturar arquitetura e pastas e definir arquitetura modular (MVC? MVVM? UI-Domain-Data? Qual é melhor para o uso de Qt Widgets?)
