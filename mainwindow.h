...
/*
First of all, BEFORE implementing the class below, answer this prompt with which files you need in order to have enough
context to implement everything without writing incorrect code.

MainWindow either instantiates or receives the data sources in the constructor (not both though; I'll leave this decision to the whims of the LLM)
...
- Hook the signal DockerEventStream::eventReceived into the reload logic (more on that later)
    - DockerEventStream::criticalError exists. If it fails the application isn't exactly broken, it just doesn't refresh automatically, so I'm unsure what to do. Make a little text in the corner?
